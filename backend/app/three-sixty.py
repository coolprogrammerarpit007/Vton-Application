    ...
    try:
        ...
        for res in trigger_results:
            if isinstance(res, Exception):
                # FIX: Properly extract the message from your custom APIException
                err_msg = res.msg if hasattr(res, 'msg') else str(res)
                logger.error(f"[360 WORKER] Failed to trigger view for Job {job_id}: {err_msg}")
                raise Exception(f"Failed to dispatch position trigger: {err_msg}")
        ...
        # Check for active subscription to process the refund
        ...
        if sub:
            # FIX: Placed inside the worker so background crashes calculate Fashn master correctly
...
async def create_360_job(
    ...
    try:
        ...
        db.rollback()
        logger.error(f"360 generation API crash: {str(e)}", exc_info=True)
        
        # FIX: Cleaned up the endpoint exception block
        if 'db_job' in locals() and getattr(db_job, 'id', None):
            db_job.status = models.JobStatus.FAILED
            db.commit()
            
            refund_params = job_params if 'job_params' in locals() else {"num_images": 1, "resolution": resolution}
            SubscriptionTransactionManager.refund_resources(
                db, subscription, cost, "three_sixty", reference_id=db_job.id, reason=str(e), params=refund_params
            )
